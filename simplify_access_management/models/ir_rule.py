...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        cid = False
        if request and request.httprequest and request.httprequest.cookies and 'cids' in request.httprequest.cookies:
            cid = request.httprequest.cookies.get('cids').split(',')[0] if request.httprequest.cookies.get(
                'cids') else False

        # if not cid:
        #     cid = request.env.company.id and str(request.env.company.id)

        if cid and self.env.user.id and read_value and not all_data:
